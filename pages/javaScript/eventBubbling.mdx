...
Calling an event on a parent element will trigger events on all child elements. In the example below, clicking on the `p` element will trigger events on the `p`, `div` and `form` elements.
